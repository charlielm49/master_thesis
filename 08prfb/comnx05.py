...
# Obtain list of unique communities
...
# Create CSR sparse matrix for adjacency list
# THIS RETURNS INCIDENCE MATRIX OF HYPERGRAPH (H)
# Iterate over the list and substitute
# IN01: (indexList: list of node adjacency list)
# [(0, 14, 1875, 98, ...), (1, 0, 986, 2097, ...), (2, 7923, 93, 12, ...), ...]
# IN02: number of edges (hyperedges)
# OUT: (matrix in CSR format)
# for this format, we need:
#  rows = []
...
def leer_texto2(fp):
    ...
    for line in fp:
        ...
        for node_dest in tmp[2:]:
            gfb.add_edge(node_src, int(node_dest))
...
def main():
    ...
    edgeList, gfb = leer_texto2(inFilePointer01)
    print "\nEdge/Graph Info"
    ...
    print "rankSortSave2 (Graph)", rankSortSave2[:10]
